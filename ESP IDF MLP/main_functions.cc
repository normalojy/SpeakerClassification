...
void setup() {
  ...
  model = tflite::GetModel(tensorflowlite_mlp_tflite);
...
void loop() {
  // if (Serial.available() >= 784) {          // 28*28 = 784 bytes
  //   // Read 784 bytes directly into the input tensor
  //   if (input->type == kTfLiteFloat32) {
  //     for (int i = 0; i < 28*28; i++) {
  //       uint8_t pixel = Serial.read();                    // 0-255
  //       input->data.f[i] = pixel / 255.0f;         // normalize to [0,1]
  //     }
  //   } 
  // }
  ...
  if (bytes_available >= 1024) {               // 28 * 28 = 784
  ...
  // // Format the prediction into a string
  // ssd1306_clear();
  // snprintf(buffer, sizeof(buffer), "Speaker: %d", predicted_speaker);
  // ssd1306_print_str(0, 0, buffer, false);
